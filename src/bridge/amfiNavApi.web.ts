/**
 * AMFI India official NAV history API — same query params as mobile; proxied for CORS.
 */
...
let amfiInflight = 0;
const amfiWaiters: Array<() => void> = [];

async function withAmfiSlot<T>(fn: () => Promise<T>): Promise<T> {
  while (amfiInflight >= AMFI_MAX_INFLIGHT) {
    await new Promise<void>((resolve) => amfiWaiters.push(resolve));
  }
  amfiInflight += 1;
  try {
    return await fn();
